...
public class Question3 {
	...
	public static Integer[] prepareArray(Integer[] array){
		...
		for (Integer i : array) {initialArray.add(i);}
		...
		while(!queue.isEmpty()) {
			...
			if (current.size() == 0) { 
				...
				queue.add(current.subList(0, (current.size()/2))); // left side
				queue.add(current.subList( (current.size()/2)+1, current.size())); // right side
			}
		}
