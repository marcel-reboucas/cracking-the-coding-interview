 ...
 * - instead of using shifts, which are expensive and not optimal, 
 *   it would be better to use two arrays: calculate the size of the 
 *   new array based on the number of the spaces and just run through
 *   it with one pass.
 */
public class Question4 {
	...
	static void shiftRight(char[] charArray, int startingPosition, int positions) {
		...
		if (startingPosition > arraySize || positions > (arraySize - startingPosition)) { return; }
